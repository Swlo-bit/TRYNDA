...
#define REDUCCION 19.72
#define LIMITE_MIN 0.0

#define LIMITE_MAX 190.0
#define HOME_REAL 185.0 
  ...
  while(!exito && reintentos < 3) { 
    ...
    if (sensorWire.endTransmission() == 0) {
      ...
      if (sensorWire.available() >= 2) { 
        ...
        if (raw <= 4095) {
          degAngle = raw * 0.087890625; 
          exito = true;
        }
      }
    }
    if(!exito) { reintentos++; sensorWire.begin(); delayMicroseconds(100); }
  }

  static int fallosConsecutivos = 0;
  if (!exito) {
    fallosConsecutivos++;
    if (fallosConsecutivos > 15 && enMovimiento) {
      enMovimiento = false;
      comandoI2C = "STOP";
      Serial.println("⚠️ AS5600 o ruido");
    }
  } else {
...
void pasoMotor(int vel) {
  digitalWrite(STEP_PIN, HIGH); delayMicroseconds(vel); digitalWrite(STEP_PIN, LOW); delayMicroseconds(vel);
}
...
void hacerHoming() {
  ...
  while (digitalRead(FIN_CARRERA) == HIGH && digitalRead(PIN_PARO) == LOW) pasoMotor(300); 
  if (digitalRead(PIN_PARO) == HIGH) return; 
  delay(300);
  ...
  digitalWrite(DIR_PIN, HIGH); 
  while (digitalRead(FIN_CARRERA) == HIGH && digitalRead(PIN_PARO) == LOW) pasoMotor(700); 
  if (digitalRead(PIN_PARO) == HIGH) return; 
  delay(500); 
  ...
  comandoI2C = "M2_STOP"; ultimoComandoI2C = "M2_STOP"; 

  objetivo = 90.0; enMovimiento = true;   
}
...
void controlMovement() {
  ...
  if (errorAbs > tolerancia) {
    estable = 0; 
    int nuevaDir = (error > 0) ? HIGH : LOW;
    
    if (nuevaDir != ultimaDir && ultimaDir != -1) { delay(40); }
    ultimaDir = nuevaDir;
...
void loop() {
  ...
  if (datoNuevoI2C) {
    datoNuevoI2C = false;
    comandoI2C = String(bufferI2C); comandoI2C.trim();
    
    if (comandoI2C == "M2_HOME") {
        homingHecho = false;  
        iniciarHoming = true; 
    }
    else if (comandoI2C != ultimoComandoI2C) {
      if (comandoI2C == "M2_FREE") { digitalWrite(ENA_PIN, HIGH); enMovimiento = false; }
      else if (comandoI2C == "M2_LOCK") { digitalWrite(ENA_PIN, LOW); objetivo = anguloFinal; enMovimiento = false; }
      ...
      else if (comandoI2C == "STOP" || comandoI2C == "M2_STOP") {
        objetivo = anguloFinal; enMovimiento = false;
      } 
      else if (comandoI2C.startsWith("GOTO_M2:")) {
        if (homingHecho) {
          float incremento = comandoI2C.substring(8).toFloat();
          objetivo = anguloFinal + incremento; enMovimiento = true; 
        } 
      }
      else if (comandoI2C.startsWith("SYNC_")) {
        int idx = comandoI2C.substring(5).toInt();
        if (idx < totalPuntos) {
          objetivo = puntosGuardados[idx];
          enMovimiento = true; 
        }
      }
  ...
  if (!iniciarHoming && homingHecho) { 
    if (comandoI2C == "M2_POSITIVE" && anguloFinal < LIMITE_MAX) { 
      digitalWrite(DIR_PIN, HIGH); for(int p = 0; p < 15; p++) pasoMotor(150); 
      actualizarPosicion(); objetivo = anguloFinal; enMovimiento = false; 
    } 
    else if (comandoI2C == "M2_NEGATIVE" && anguloFinal > LIMITE_MIN) { 
      digitalWrite(DIR_PIN, LOW); for(int p = 0; p < 15; p++) pasoMotor(150); 
      actualizarPosicion(); objetivo = anguloFinal; enMovimiento = false; 
    }
  }
