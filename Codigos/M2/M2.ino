...
#define LIMITE_MIN -43.0
#define LIMITE_MAX 195.0
#define HOME_REAL -47.0
...
void actualizarPosicion() {
  ...
  if (homingHecho) anguloFinal = anguloSalida - offset; 
  else anguloFinal = anguloSalida;
}
...
void hacerHoming() {
  ...
  while (digitalRead(FIN_CARRERA) == HIGH && digitalRead(PIN_PARO) == LOW) pasoMotor(850);
  ...
  digitalWrite(DIR_PIN, HIGH); 
  while (digitalRead(FIN_CARRERA) == HIGH && digitalRead(PIN_PARO) == LOW) pasoMotor(3000);
  if (digitalRead(PIN_PARO) == HIGH) return; // Aborta
  delay(500); 
  ...
  comandoI2C = "M3_STOP"; ultimoComandoI2C = "M3_STOP"; 
  objetivo = 90.0; 
  enMovimiento = true;   
...
void controlMovement() {
  ...
  if (errorAbs > tolerancia) {
    ...
    if (errorAbs > 5.0) { vel = velMax; pasos = 8; } 
    else if (errorAbs > 2.0) { vel = 1200; pasos = 3; } 
    else { vel = 1800; pasos = 2; }
...
void loop() {
  ...
  if (datoNuevoI2C) {
    ...
    if (comandoI2C == "M3_HOME") {
        homingHecho = false;
        iniciarHoming = true;
    } 
    else if (comandoI2C != ultimoComandoI2C) {
      if (comandoI2C == "M3_FREE") { digitalWrite(ENA_PIN, HIGH); enMovimiento = false; }
      else if (comandoI2C == "M3_LOCK") { digitalWrite(ENA_PIN, LOW); objetivo = anguloFinal; enMovimiento = false; }
      ...
      else if (comandoI2C == "STOP" || comandoI2C == "M3_STOP") {
        objetivo = anguloFinal; enMovimiento = false;
      } 
      else if (comandoI2C.startsWith("GOTO_M3:")) {
        if (homingHecho) {
